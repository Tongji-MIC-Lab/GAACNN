...
import AE
import Model.model_new as model
from Model.context_model import Weighted_Gaussian
from Util.metrics import evaluate

GPU = True
# index - [0-15]
models = ["mse800", "mse1200", "mse1600", "mse2400", "mse3200", "mse6400","mse12800", "mse25600","mse51200","mse102400","mse204800",
           "msssim180", "msssim360", "msssim720", "msssim1440", "msssim2880"]

@torch.no_grad()
def inference_rd(im_dirs, out_dir, model_dir, model_index, block_width, block_height):
    M, N2 = 192, 128
    if (model_index == 7) or (model_index == 8) or (model_index == 9) or (model_index == 10) or (
            model_index == 13) or (model_index == 14) or (model_index == 15):
        M, N2 = 320, 128
    image_comp = model.Image_coding(3, M, N2)
    ...
    for im_dir in im_dirs:
        ...
        bin_dir = os.path.join(out_dir,os.path.splitext(os.path.basename(im_dir))[0]+'enc.bin')
        ...
        for img in img_block_list:
            ...
            if GPU:
                im = im.cuda()
            print('====> Encoding Image:', im_dir, "%dx%d" % (block_H, block_W), 'to', bin_dir,
            ...
            with torch.no_grad():
                ...
                y_main_q = torch.Tensor(y_main_q.cuda().data.cpu().numpy().astype(np.int))
                ...
                y_hyper_q = torch.Tensor(y_hyper_q.cuda().data.cpu().numpy().astype(np.int))
            ...
            cdf_m = cdf_m.astype(np.int32) + sample.cuda().data.cpu().numpy().astype(np.int32) - Min_Main
