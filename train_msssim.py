...
def adjust_learning_rate_flicker(optimizer,step_count):
    if step_count<200000:
        lr=5e-5
    elif step_count < 300000:
        lr = 2.5e-5
    elif step_count < 400000:
        lr = 1.25e-5
    elif step_count < 500000:
        lr = 5e-6
    elif step_count < 600000:
        lr = 2.5e-6
    else:
        lr = 1e-6
...
def train(args):
    # NIC_Dataset
    train_data = ImageFolder('/root/dataset/flicker_2W_images_crop256', transform=transforms.Compose(
        [transforms.ToTensor()]))
    train_loader = DataLoader(train_data, batch_size=16, shuffle=True, num_workers=8)

    image_comp = model.Image_coding(3, args.M, args.N2).cuda()
    context = Weighted_Gaussian(args.M).cuda()

    model_existed = os.path.exists(os.path.join(args.out_dir, 'msssim'+str(int(args.lmbda*100))+r'.pkl')) and \
        os.path.exists(os.path.join(args.out_dir, 'msssim'+str(int(args.lmbda*100))+r'p.pkl'))
    if model_existed:
        image_comp.load_state_dict(torch.load(os.path.join(args.out_dir, 'msssim'+str(int(args.lmbda*100))+r'.pkl')))
        context.load_state_dict(torch.load(os.path.join(args.out_dir, 'msssim'+str(int(args.lmbda*100))+r'p.pkl')))
    ...
    for epoch in range(0,15):
        ...
        for step, batch_x in enumerate(train_loader):
            ...
            if (epoch < 1) and (not model_existed):
                dloss = mse_loss
                lamb = args.lmbda * 100
            else:
                dloss = 1.0 - msssim
            ...
            l_rec = lamb * dloss + 0.1 * train_bpp3 + 0.1 * train_bpp2
            ...
            mse_tmp += mse_loss.item()
            msssim_tmp += msssim.item()
            train_bpp3_tmp += train_bpp3.item()
            train_bpp2_tmp += train_bpp2.item()
            if step % 100 == 0:
                with open(os.path.join(args.out_dir, 'train_msssim'+str(int(args.lmbda*100))+'.log'),'a') as fd:
                    time_used = time.time()-last_time
                    last_time = time.time()
                    mse = mse_tmp / (step+1)
                    psnr1 = 10.0 * np.log10(1./mse)
                    msssim_dB = -10*np.log10(1-(msssim_tmp/(step+1)))
                    bpp_total = (train_bpp3_tmp + train_bpp2_tmp) / (step+1)
                    fd.write('ep:%d step:%d time:%.1f cur_lr:%.8f loss:%.6f MSE:%.6f bpp_main:%.4f bpp_hyper:%.4f bpp_total:%.4f psnr:%.2f msssim:%.2f\n'
                             %(epoch, step_count, time_used, cur_lr, rec_loss_tmp/(step+1), mse, train_bpp3_tmp/(step+1), train_bpp2_tmp/(step+1), bpp_total, psnr1, msssim_dB))
                    # print('ep:%d step:%d time:%.1f lr:%.8f loss:%.6f MSE:%.6f bpp_main:%.4f bpp_hyper:%.4f bpp_total:%.4f psnr:%.2f msssim:%.2f\n'
                    #     % (epoch, step, time_used, cur_lr, rec_loss_tmp / (step + 1), mse, train_bpp3_tmp / (step + 1),
                    #        train_bpp2_tmp / (step + 1), bpp_total, psnr1, msssim_dB))
                fd.close()
            ...
            if (step+1) % 2000 == 0:
                torch.save(context.module.state_dict(),
                           os.path.join(args.out_dir, 'msssim'+str(int(args.lmbda*100))+r'p.pkl'))
                torch.save(image_comp.module.state_dict(),
                           os.path.join(args.out_dir, 'msssim'+str(int(args.lmbda*100))+r'.pkl'))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lambda", type=float, default=32, dest="lmbda", help="Lambda for rate-distortion tradeoff.")
